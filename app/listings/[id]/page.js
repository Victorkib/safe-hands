...
export default function ListingDetail() {
  ...
  const fetchRelatedListings = async (sellerId, currentListingId) => {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        const related = result.listings.filter(
          l => l.seller_id === sellerId && l.id !== currentListingId && l.status === 'active'
        ).slice(0, 4);
  ...
  const handleBuyNow = () => {
    // Pre-fill transaction data and redirect
    ...
    // Store in localStorage to pre-fill form
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        <Link href="/marketplace" className="text-blue-600 hover:text-blue-700 mb-4 inline-block">
          ← Back to Marketplace
        </Link>
      </div>

...
import {
  ...
  getAutomatedSuggestionBlockReason,
  getResponseStatusSummary,
...
export function isEligibleForBulkSuggestionResolve(dispute) {
  ...
  if (!canApplyAutomatedSuggestion(dispute)) return false;
...
export function getBulkIneligibleReason(dispute) {
  ...
  const responseBlock = getAutomatedSuggestionBlockReason(dispute);
...
/** Same response-window gate as bulk — used for Quick resolve buttons. */
export function isEligibleForQuickSuggestionResolve(dispute) {
  if (!isEligibleForSuggestionResolve(dispute)) return false;
  return canApplyAutomatedSuggestion(dispute);
}

export function getQuickResolveBlockReason(dispute) {
  if (!isEligibleForSuggestionResolve(dispute)) return 'No system suggestion available';
  return getAutomatedSuggestionBlockReason(dispute);
}

export { getResponseStatusSummary, canApplyAutomatedSuggestion };
...
export function downloadSuggestionSummaryCsv(disputes, filename) {
  ...
  const rows = list.map((d) => {
    ...
    const responseSummary = getResponseStatusSummary(d);
    return [
      d.id,
      d.status,
      ...
      d.response_due_at ? new Date(d.response_due_at).toISOString() : '',
      d.accused_responded_at ? new Date(d.accused_responded_at).toISOString() : '',

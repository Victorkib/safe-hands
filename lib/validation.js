...
// Supports both camelCase (sellerEmail) and snake_case (seller_email) field names
export const validateTransactionForm = (formData) => {
  const errors = {};

  // Support both field naming conventions
  const sellerEmail = formData.sellerEmail || formData.seller_email;
  
  if (!sellerEmail) {
    errors.seller_email = 'Seller email is required';
...
export const validateListingForm = (formData) => {
  ...
  } else if (parseFloat(formData.price) > 99999999) {
    errors.price = 'Price cannot exceed KES 99,999,999';
  }

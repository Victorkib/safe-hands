...
class MPesaClient {
  ...
  async initiateSTKPush(params) {
    try {
      ...
      if (!this.callbackURL || !String(this.callbackURL).trim()) {
        throw new Error(
          'MPESA_CALLBACK_URL is not set — STK callbacks cannot be delivered. Set it to your public /api/mpesa/callback URL.'
        );
      }
  ...
  async initiateB2C(params) {
    try {
      ...
      const qUrl = (queueTimeOutURL || this.callbackURL || '').trim();
      const rUrl = (resultURL || this.callbackURL || '').trim();
      if (!qUrl || !rUrl) {
        throw new Error(
          'B2C requires QueueTimeOutURL and ResultURL. Set NEXT_PUBLIC_APP_URL (or MPESA_B2C_RESULT_URL / MPESA_CALLBACK_URL) so callbacks can reach this app.'
        );
      }
      ...
      const response = await axios.post(
        ...
        {
          OriginatorConversationID: originatorConversationId || this.generateConversationID(),
          ...
          Remarks: (remarks || 'Escrow fund release').slice(0, 100),
          QueueTimeOutURL: qUrl,
          ResultURL: rUrl,
        },
        {
          headers: {

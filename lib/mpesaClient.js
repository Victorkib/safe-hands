...
class MPesaClient {
  ...
  async initiateB2C(params) {
    try {
      ...
      // Use separate B2C callback URL
      const b2cCallbackURL = process.env.MPESA_B2C_CALLBACK_URL || `${process.env.NEXT_PUBLIC_APP_URL}/api/mpesa/b2c-callback`;
      ...
      const response = await axios.post(
        `${this.baseURL}/mpesa/b2c/v3/paymentrequest`,
        {
          ...
          Amount: Math.floor(amount),
          PartyA: process.env.MPESA_B2C_SHORT_CODE || process.env.MPESA_SHORT_CODE || '174379',

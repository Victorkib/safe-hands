...
import { sendDisputeResolvedEmails } from '@/lib/disputeResolvedEmail';
...
export async function resolveDisputeAsAdmin(supabase, input) {
  ...
  let emailDelivery = { ok: false, results: [] };
  try {
    emailDelivery = await sendDisputeResolvedEmails(supabase, {
      resolution,
      amount,
    ...
    console.error('Dispute resolved emails failed:', emailErr);
    emailDelivery = { ok: false, error: emailErr?.message || 'Email send failed', results: [] };
  }

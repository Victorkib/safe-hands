...
// FIX: wrong import path. Use server-side admin client, not browser client
...
export async function verifyEmailToken(token) {
  try {
    ...
    if (userUpdateError) {
      console.error(
        '[v0] Error updating user email verification status:',
        userUpdateError,
      );
...
export async function cleanupExpiredTokens() {
  try {
    ...
    if (verificationError) {
      console.error(
        '[v0] Error cleaning verification tokens:',
        verificationError,
      );
    ...
    console.log(
      `[v0] Cleaned up ${verificationCount || 0} verification tokens and ${resetCount || 0} reset tokens`,
    );

...
/**
 * Send via Mailjet REST API (backup when Gmail is unavailable).
...
/**
 * Send email using Gmail (primary) with Mailjet fallback.
...
export async function sendEmail({ to, subject, html, text }) {
  try {
    ...
    console.warn('[email] Gmail failed, trying Mailjet:', gmailResult.error);

    const mailjetResult = await sendViaMailjet({ to, subject, html, text });
    if (mailjetResult.success) return mailjetResult;

    const combinedError = [gmailResult.error, mailjetResult.error].filter(Boolean).join(' | ');
    throw new Error(combinedError || 'No email service available');
  } catch (error) {
